...
class LaptopDataExplorer:
    ...
    def __init__(self, data_path: str = None):
        ...
        self.valid_ram_range = (4, 64)  # 4GB to 64GB
        self.valid_storage_range = (256, 4096)  # 256GB to 4TB
        self.load_data()
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        print(f"Laptop data columns: {self.df_laptop.columns}")
    ...
    def identify_and_clean_invalid_specifications(self) -> Dict:
        ...
        # Use class-defined valid ranges
        if 'ram_gb' in self.df_laptop.columns:
            ram_mask = (
                (self.df_laptop['ram_gb'] < self.valid_ram_range[0]) | 
                (self.df_laptop['ram_gb'] > self.valid_ram_range[1])
        ...
        if 'storage_gb' in self.df_laptop.columns:
            storage_mask = (
                (self.df_laptop['storage_gb'] < self.valid_storage_range[0]) | 
                (self.df_laptop['storage_gb'] > self.valid_storage_range[1])
        ...
        if invalid_indices:
            ...
            invalid_records['summary'] = {
                ...
                'valid_ram_range': self.valid_ram_range,  # Use class attribute
                'valid_storage_range': self.valid_storage_range,  # Use class attribute
    ...
    def get_invalid_records_details(self) -> pd.DataFrame:
        ...
        for idx in invalid_df.index:
            ...
            if invalid_df.loc[idx, 'ram_invalid']:
                ...
                notes.append(f"RAM {ram_val}GB outside valid range ({self.valid_ram_range[0]}-{self.valid_ram_range[1]}GB)")
            if invalid_df.loc[idx, 'storage_invalid']:
                storage_val = invalid_df.loc[idx, 'storage_gb']
                notes.append(f"Storage {storage_val}GB outside valid range ({self.valid_storage_range[0]}-{self.valid_storage_range[1]}GB)")
    ...
    def clean_invalid_specifications(self, action: str = 'drop') -> Dict:
        ...
        Updated with new valid ranges for RAM and storage.
        ...
        if 'ram_gb' in self.df_laptop.columns:
            ...
            invalid_indices.update(self.df_laptop[ram_mask].index)
        ...
        if 'storage_gb' in self.df_laptop.columns:
            ...
            invalid_indices.update(self.df_laptop[storage_mask].index)
        ...
        if action == 'drop':
            ...
            self.df_laptop['is_valid'] = ~self.df_laptop.index.isin(invalid_indices)
            return {
                'action': 'mark',
                'records_affected': len(invalid_indices),
                'validation_flag_added': 'is_valid'
            }
        else:
            return {'error': f'Invalid action: {action}. Use "drop" or "mark".'}

    def create_json_for_analytics(self) -> Dict:
        """
        Create a JSON-compatible dictionary for analytics dashboard.
        """
        if self.df_laptop is None:
        ...
        if 'brand_original' in self.df_laptop.columns:
            brands = self.df_laptop['brand_original'].fillna('Unknown')
            # Clean and format brand names
            brands = brands.apply(lambda x: str(x).strip().title())
    ...
    def create_visualizations_for_web(self, save_path: str = "static/images/"):
        """
        Create visualizations specifically for the web analytics dashboard.
        """
        os.makedirs(save_path, exist_ok=True)
        ...
        plt.hist(self.df_laptop['price'].dropna(), bins=50, alpha=0.7, edgecolor='black')
        plt.title('Price Distribution')
        plt.xlabel('Price (RM)')
        plt.ylabel('Frequency')
        plt.savefig(f"{save_path}price_distribution.png", dpi=300, bbox_inches='tight')
        plt.close()
        
        # 2. RAM Distribution
        plt.figure(figsize=(10, 6))
        self.df_laptop['ram_gb'].value_counts().sort_index().plot(kind='bar')
        plt.title('RAM Distribution')
        plt.xlabel('RAM (GB)')
        plt.ylabel('Count')
        plt.savefig(f"{save_path}ram_distribution.png", dpi=300, bbox_inches='tight')
        plt.close()
        
        # 3. Storage Distribution
        plt.figure(figsize=(10, 6))
        self.df_laptop['storage_gb'].value_counts().sort_index().plot(kind='bar')
        plt.title('Storage Distribution')
        plt.xlabel('Storage (GB)')
        plt.ylabel('Count')
        plt.savefig(f"{save_path}storage_distribution.png", dpi=300, bbox_inches='tight')
        plt.close()
        
        # 4. Brand Distribution
        plt.figure(figsize=(12, 6))
        self.df_laptop['brand'].value_counts().head(10).plot(kind='bar')
        plt.title('Top 10 Brands')
        plt.xlabel('Brand')
        plt.ylabel('Count')
        plt.xticks(rotation=45)
        plt.savefig(f"{save_path}brand_distribution.png", dpi=300, bbox_inches='tight')
        plt.close()
